...
def always_change(verbose=False):
    """
    Simulate a single play of the Monty Hall Problem where, after a losing
    door is eliminated, the player always changes their selected door.
    ...
    if verbose:
        print(
            f"You first chose {contestant_door}. We then eliminated {discarded_door}. You switched to {new_contestant_door}. The car was behind {prize_door}. ",
            end="",
        )
        if winner:
            print("YOU WIN!!!")
